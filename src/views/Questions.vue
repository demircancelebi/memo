          ...
          <div class="col-4">
            <div class="card">
              <div class="card-body p-5">
                ...
                <p class="lead mt-3 mb-0">They will show up here as you add them</p>
              </div>
            </div>
          </div>
        </div>
      ...
      <table class="table table-striped table-bordered" v-if="questions.length > 0">
        ...
        <tbody>
          <tr v-for="q, qIndex in questions" :key="q.q">
            ...
            <td class="align-middle" v-if="false">{{ q.do_not_show_before }}</td>
            <td class="align-middle">
              <span v-for="t, tagIndex in q.tags"
                :key="t"
                @click.prevent="removeTag(q.tags, tagIndex)">
                <span :class="{ 'can-be-deleted': q.tags.length > 1 }">{{ t }}</span>&nbsp;
              </span>
            </td>
            <td class="align-middle">
              <a href="#" class="action-button text-secondary me-2" v-if="false">
                <span class="fa fa-edit"></span>
              </a>
              <a href="#" class="action-button text-danger" @click.prevent="removeQuestion(qIndex)">
